...
public class DockerService : IDockerService, IDisposable
{
    ...
    public async Task<string> BuildImageAsync(string contextPath, string imageName, string dockerfilePath = "Dockerfile")
    {
        _logger.LogInformation("Building Docker image: {Image} from {Context}", imageName, contextPath);
        ...
        var result = await Cli.Wrap("docker")
            .WithArguments(new[]
            {
                "build",
                "-t", imageName,
                "-f", Path.Combine(contextPath, dockerfilePath),
                contextPath
            })
            .WithValidation(CommandResultValidation.None)
            .ExecuteBufferedAsync();
        ...
        if (result.ExitCode != 0)
        {
            _logger.LogError("Docker build failed: {Error}", result.StandardError);
            throw new InvalidOperationException($"Docker build failed: {result.StandardError}");
        }

        _logger.LogInformation("Docker image built: {Image}", imageName);
        return result.StandardOutput;
    }
