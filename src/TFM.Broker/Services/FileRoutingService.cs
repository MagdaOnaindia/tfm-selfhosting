...
public class FileRoutingService : IRoutingService
{
    ...
    public async Task AddRouteAsync(string domain, string agentId, int targetPort, string? description = null)
    {
        DomainMappingConfiguration config;
        if (File.Exists(_configPath))
        {
            ...
            config = JsonSerializer.Deserialize<DomainMappingConfiguration>(json, options) ?? new();
        }
        else
        {
            config = new DomainMappingConfiguration();
        }

        config.Routes[domain] = new DomainRoute
        {
            AgentId = agentId,
        ...
        _logger.LogInformation("Ruta agregada: {Domain} -> Agent {AgentId}:{Port}", domain, agentId, targetPort);

        await ReloadRoutesAsync();
    }

    public async Task RemoveRouteAsync(string domain)
    {
        DomainMappingConfiguration config;
        if (File.Exists(_configPath))
        {
        ...
        if (config.Routes.Remove(domain))
        {
            var writeOptions = new JsonSerializerOptions { WriteIndented = true };
            var updatedJson = JsonSerializer.Serialize(config, writeOptions);
            await File.WriteAllTextAsync(_configPath, updatedJson);
            _logger.LogInformation("Ruta eliminada: {Domain}", domain);
            await ReloadRoutesAsync();
        }
        else
        {
            _logger.LogWarning("No se encontró ruta para eliminar: {Domain}", domain);
        }
    }
