...
// Catch-all: cualquier petición que no coincida con /health o /admin/*
// se trata como proxy hacia el Agent a través del túnel gRPC
app.MapFallback("{*path}", async (
    HttpContext context,
    ...
    IRoutingService routingService) =>
    {
        ...
        {
            // No reenviar Accept-Encoding: el contenido viaja sin comprimir por el túnel
            // Caddy se encarga de comprimir en el tramo final al navegador
        ...
        {
            const long maxBodySize = 16 * 1024 * 1024; // 16 MB
        ...
        try
        {
            ...
            // Devolver respuesta (filtrar hop-by-hop headers que son ilegales en HTTP/2)
...
// Middleware de autenticaci�n para endpoints admin
...
app.MapPost("/admin/routes", async (HttpContext context, IRoutingService routingService) =>
{
    if (!context.Request.Headers.TryGetValue("X-API-Key", out var apiKey) || apiKey != adminApiKey)
    {
        return Results.Unauthorized();
    ...
    var body = await context.Request.ReadFromJsonAsync<AddRouteRequest>();
    if (body == null || string.IsNullOrWhiteSpace(body.Domain) || string.IsNullOrWhiteSpace(body.AgentId))
    {
        return Results.BadRequest(new { error = "domain and agentId are required" });
    }

    if (!IsValidDomain(body.Domain))
    {
        return Results.BadRequest(new { error = "Invalid domain format" });
    }

    await routingService.AddRouteAsync(body.Domain, body.AgentId, body.TargetPort, body.Description);
    return Results.Ok(new { message = $"Route added: {body.Domain} -> {body.AgentId}" });
});

app.MapDelete("/admin/routes/{domain}", async (string domain, HttpContext context, IRoutingService routingService) =>
{
    if (!context.Request.Headers.TryGetValue("X-API-Key", out var apiKey) || apiKey != adminApiKey)
    {
        return Results.Unauthorized();
    ...
    await routingService.RemoveRouteAsync(domain);
    return Results.Ok(new { message = $"Route removed: {domain}" });
});

bool IsHopByHopHeader(string name) =>
...
bool IsValidDomain(string domain)
{
    ...
    // RFC 1123 - solo letras, n�meros, puntos y guiones
...
record AddRouteRequest(string Domain, string AgentId, int TargetPort = 80, string? Description = null);